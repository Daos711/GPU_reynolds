"""
Бенчмарк: сравнение производительности CPU (Numba) vs GPU (CuPy Red-Black SOR)
на разных размерах сетки.

Запуск:
    python -m gpu_reynolds.benchmark
"""
...
# ───────────────────────────────────────────────────────────────────────────
# CPU-солвер (Numba) — референсная реализация
# ───────────────────────────────────────────────────────────────────────────
@njit
...
def generate_test_H(N_Z, N_phi, epsilon=0.6):
    """Генерирует тестовый зазор H = 1 + epsilon * cos(phi)."""
...
def run_benchmark():
    ...
    # Прогрев JIT Numba на маленькой сетке
    ...
    # Импортируем GPU-солвер
    try:
        import cupy as cp
        from gpu_reynolds.solver import solve_reynolds_gpu

        # Прогрев GPU (компиляция ядра + аллокации)
        print("Прогрев GPU...")
