...
W_hs = float(np.sqrt(
    np.trapz(np.trapz(P_hs * np.cos(Phi), phi_1D, axis=1), Z_1D)**2 +
    np.trapz(np.trapz(P_hs * np.sin(Phi), phi_1D, axis=1), Z_1D)**2
)) * R * L / 2

print(f"W_hs = {W_hs:.0f} N")
print()
...
for max_out in [100, 300, 500, 1000]:
    ...
    W_jfo = float(np.sqrt(
        np.trapz(np.trapz(P_jfo * np.cos(Phi), phi_1D, axis=1), Z_1D)**2 +
        np.trapz(np.trapz(P_jfo * np.sin(Phi), phi_1D, axis=1), Z_1D)**2
    )) * R * L / 2

    cav_frac = float(np.mean(theta < 1.0 - 1e-6))
    converged = (n_out < max_out)
    results.append((max_out, W_jfo, cav_frac, n_out, converged))
    print(f"max_outer={max_out:>5d}: W={W_jfo:>8.0f} N, "
...
spread = (max(W_values) - min(W_values)) / np.mean(W_values)
